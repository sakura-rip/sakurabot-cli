...
func (p *chargeParams) processInteract(args []string) {
	uid, err := actor.PromptAndRetry("user id ", actor.CheckIsAPositiveNumber, func(s string) error {
	...
	amount, err := actor.PromptAndRetry("amount", func(s string) error {
	...
	type_, err := actor.PromptOptional("type", "amazon")
	if err != nil {
		logger.Fatal().Err(err).Msg("")
	}
	p.chargeType = type_
